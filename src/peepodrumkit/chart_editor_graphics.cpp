...
namespace PeepoDrumKit
{
	...
	struct RasterizedBitmap
	{
		std::unique_ptr<u32[]> BGRA;
		ivec2 Resolution;
	};
	struct SvgRasterizer
	{
		...
		void ParseSVG(std::string_view svgFileContent, f32 baseScale)
		{
			...
			auto result = picture->load(svgFileContent.data(), static_cast<u32>(svgFileContent.size()), "svg", nullptr, true);
			result = picture->size(&PictureSize.x, &PictureSize.y);
		...
		{
			tvg::Result result = tvg::Result::Success;
			auto picture = tvg::Picture::gen();
			result = picture->load(imagePath.c_str());
			result = picture->size(&PictureSize.x, &PictureSize.y);
			PictureSize *= baseScale;
			BaseScale = baseScale;
			PictureView = picture;
		...
		RasterizedBitmap Rasterize(f32 scale)
		{
			...
			result = PictureView->scale(scale * BaseScale);
			result = PictureView->translate(position.x, position.y);

			result = Canvas->target(out.BGRA.get(), resolution.x, resolution.x, resolution.y, tvg::ColorSpace::ARGB8888 /*_STRAIGHT*/);
			result = Canvas->push(PictureView);
			result = Canvas->update(PictureView);
			result = Canvas->draw();
			result = Canvas->sync();

			if constexpr (PerSideRasterizedTexPadding > 0)
			{
				auto pixelAt = [&](i32 x, i32 y) -> u32 &
				{ return out.BGRA[(y * resolution.x) + x]; };
				auto pixelAtWithoutPadding = [&](i32 x, i32 y) -> u32 &
				{ return pixelAt(x + PerSideRasterizedTexPadding, y + PerSideRasterizedTexPadding); };
	...
	ChartGraphicsResources::ChartGraphicsResources()
	{
		...
		auto result = tvg::Initializer::init(threadCount, tvg::CanvasEngine::Sw);
	...
	ChartGraphicsResources::~ChartGraphicsResources()
	{
		for (auto &it : Data->PerSprTexture)
		{
			it.Unload();
		}
	}
	...
	SprStretchtOut StretchMultiPartSpr(ChartGraphicsResources &gfx, SprID spr, SprTransform transform, u32 color, SprStretchtParam param, size_t splitCount)
	{
		...
		if (splitCount == 2)
		{
			...
			ImImageQuad baseQuad{};
			gfx.GetImageQuad(baseQuad, spr, transform, color, nullptr);
			...
			out.Quads[0].TR(topM);
			out.Quads[0].UV_TR(uvTopM);
			out.Quads[0].BR(botM);
			out.Quads[0].UV_BR(uvBotM);

			out.Quads[1] = baseQuad;
			out.Quads[1].TL(topM);
			out.Quads[1].UV_TL(uvTopM);
			out.Quads[1].BL(botM);
			out.Quads[1].UV_BL(uvBotM);
		}
		else if (splitCount == 3)
		{
			...
			out.BaseTransform = transform;
			out.Quads[0] = baseQuad;
			out.Quads[0].TR(topLM);
			out.Quads[0].UV_TR(uvTopLM);
			out.Quads[0].BR(botLM);
			out.Quads[0].UV_BR(uvBotLM);

			out.Quads[1] = baseQuad;
			out.Quads[1].TL(topLM);
			out.Quads[1].UV_TL(uvTopLM);
			out.Quads[1].BL(botLM);
			out.Quads[1].UV_BL(uvBotLM);
			out.Quads[1].TR(topRM);
			out.Quads[1].UV_TR(uvTopRM);
			out.Quads[1].BR(botRM);
			out.Quads[1].UV_BR(uvBotRM);

			out.Quads[2] = baseQuad;
			out.Quads[2].TL(topRM);
			out.Quads[2].UV_TL(uvTopRM);
			out.Quads[2].BL(botRM);
			out.Quads[2].UV_BL(uvBotRM);
		}
		else
		{
			assert(false);
