...
namespace PeepoDrumKit
{
	...
	enum class LoadSettingsResponse
	{
		FileNotFound,
		AllGood,
		ErrorAbort,
	...
	static LoadSettingsResponse ReadParseSettingsIniFile(cstr iniFilePath, T &out)
	{
		...
		if (result == Shell::MessageBoxResult::Abort)
			return LoadSettingsResponse::ErrorAbort;
		if (result == Shell::MessageBoxResult::Retry)
			return LoadSettingsResponse::ErrorRetry;
		if (result == Shell::MessageBoxResult::Ignore)
			return LoadSettingsResponse::ErrorIgnore;
		assert(!"Unreachable");
		return LoadSettingsResponse::ErrorAbort;
	}
	...
	int EntryPoint()
	{
		...
		while (true)
		{
			...
			if (response == LoadSettingsResponse::FileNotFound)
			{
				break;
			}
			if (response == LoadSettingsResponse::AllGood)
			{
				break;
			}
			if (response == LoadSettingsResponse::ErrorAbort)
			{
				return -1;
			}
			if (response == LoadSettingsResponse::ErrorRetry)
			{
				continue;
			}
			if (response == LoadSettingsResponse::ErrorIgnore)
			{
				break;
			}
			assert(!"Unreachable");
		...
		callbacks.OnShutdown = []
		{
			...
			iniFileContent.reserve(4096);
			SettingsToIni(PersistentApp, iniFileContent);
			File::WriteAllBytes(PersistentAppIniFileName, iniFileContent);

			if (Settings.IsDirty)
			{
				iniFileContent.clear();
				SettingsToIni(Settings, iniFileContent);
				File::WriteAllBytes(SettingsIniFileName, iniFileContent);
